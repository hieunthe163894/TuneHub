...
export default function ReportComment() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await OriginalRequest(`reportquestion/getReport/${id}`, "GET");
                if (data && data.data) {
        ...
        <div className="max-w-full mx-auto mt-10 p-5 shadow-sm shadow-neutral-400 dark:shadow-blue-800 dark:shadow-sm rounded-lg bg-light5 dark:bg-dark30 text-lightText dark:text-darkText">
            <div className="border-l-2 border-gray-300 pl-4 mb-2">
                ...
                <div className="flex items-center mt-2">
                    <img
                        src={comment[0].userReport.profile_picture || "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png"}
                        alt=""
                        className="w-14 h-14 rounded-full mr-3 object-cover"
                    />
                    <div className="w-32 mr-4 ml-2 font-semibold">
                        {`${comment[0].userReport.first_name} ${comment[0].userReport.last_name}`}
                    </div>
